...
class _ConfigDialogState extends State<ConfigDialog> {
  ...
  Future<void> _fetchFromServer() async {
    try {
      ...
      final result = await VisorConfigService().fetchAndSaveConfig(
        onProgress: (current, total) {
          if (mounted) {
            setState(() {
              _fetchStatus = 'Descargando imágenes grupo $current/$total...';
            });
          }
        },
      );
      ...
      if (mounted) {
        ...
        if (result.validImages > 0) {
          final tipo = config.esLink == 1 ? 'Enlace' : 'Base64';
          imageInfo =
  ...
  List<Widget> _buildActions() {
    return [
      TextButton(
        onPressed: _isFetching ? null : _fetchFromServer,
        child: Text(
          _isFetching ? 'Descargando...' : 'Actualizar desde Servidor',
        ),
      ),
      TextButton(
        onPressed: () async {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: const Text('Configuración'),
      content: SingleChildScrollView(child: _buildFormContent()),
